...
class TimeSeriesChartView @JvmOverloads constructor(
    ...
    /**
     * One point on a discharge curve. Carried here rather than in a record type
     * because the app keeps the last result in memory instead of archiving it;
     * [ResultActivity] builds these straight from the engine's sample log.
     */
    ...
    private var points: List<TimePoint> = emptyList()
    ...
    fun setData(pts: List<TimePoint>) {
        points = pts
    ...
    /** The series this chart can draw, for a caller building its own legend. */
    fun seriesList(): List<Series> = seriesDefs(true)

    fun isSeriesOn(key: Char): Boolean = key in visible
    ...
    private fun valueOf(p: TimePoint, key: Char): Float = when (key) {
    ...
    fun drawInto(
        ...
        for (k in 0..4) {
            ...
            text.textAlign = Paint.Align.CENTER
            canvas.drawText(DeviceCore.formatDuration(tick), gx, plot.bottom + dp(13f), text)
        }
        ...
        if (interactive) cursorT?.let { ct ->
            ...
            canvas.drawText("t ${DeviceCore.formatDuration(nearest.tMs / 1000)}", tx, ty, text)
            for (def in defs) {
                ty += dp(13f)
