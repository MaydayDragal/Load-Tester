...
object El15Protocol {
    ...
    // parser enforces on incoming packets. A real EL15 silently drops any
    // command whose bytes don't sum to zero.
    //
    // The DM40GUI reference these were ported from got this wrong for everything
    // but POLL: its command constants were hand-written as prefix+value with NO
    // checksum byte, so on real hardware POLL worked (its 0x3F checksum was
    // captured whole) while load/mode/setpoint were silently dropped. Verified
    // on a real unit 2026-07-24 and fixed in the ESP firmware the same day; this
    // is that fix ported back. Every frame below is now *built* through
    // [withChecksum] rather than hand-written, so the byte cannot drift again.

    /** Trailing checksum byte that makes a frame sum to 0 (mod 256). */
    fun frameChecksum(data: ByteArray): Byte {
        val sum = data.sumOf { it.toInt() and 0xFF }
        return ((0x100 - (sum and 0xFF)) and 0xFF).toByte()
    }

    /** Append the sum-to-zero checksum byte the EL15 requires. */
    fun withChecksum(frame: ByteArray): ByteArray = frame + frameChecksum(frame)

    /** Poll for a fresh status packet. */
    val POLL: ByteArray = withChecksum(byteArrayOf(0xAF.toByte(), 0x07, 0x03, 0x08, 0x00))
    val LOAD_ON: ByteArray = withChecksum(byteArrayOf(0xAF.toByte(), 0x07, 0x03, 0x09, 0x01, 0x04))
    val LOAD_OFF: ByteArray = withChecksum(byteArrayOf(0xAF.toByte(), 0x07, 0x03, 0x09, 0x01, 0x00))
    val LOCK: ByteArray = withChecksum(byteArrayOf(0xAF.toByte(), 0x07, 0x03, 0x09, 0x01, 0x01))
    ...
    // ---- Command builders -------------------------------------------------
    // Both append the sum-to-zero checksum, or the load ignores the frame.
    fun modeCommand(mode: Int): ByteArray =
        withChecksum(MODE_PREFIX + byteArrayOf(mode.toByte()))
    ...
    fun setpointCommand(value: Float): ByteArray {
        ...
        return withChecksum(SETPOINT_PREFIX + f)
    }
...
class El15Status {
    ...
    /**
     * Field-for-field copy. Used at the probe-compensation boundary
     * ([DeviceCore.compensateProbe]), which must hand the corrected reading
     * downstream without mutating the frame the R-Test engine still needs raw.
     */
    fun copyOf(): El15Status {
        val c = El15Status()
        c.raw = raw; c.crcPass = crcPass; c.valid = valid
        c.voltage = voltage; c.current = current; c.power = power
        c.runtime = runtime; c.temperature = temperature; c.setpoint = setpoint
        c.energyWh = energyWh; c.capacityAh = capacityAh
        c.dcrMilliOhm = dcrMilliOhm; c.dcrI1 = dcrI1; c.dcrI2 = dcrI2
        c.mode = mode; c.modeName = modeName; c.fanSpeed = fanSpeed
        c.loadOn = loadOn; c.lockOn = lockOn; c.ready = ready
        c.setpointUnit = setpointUnit; c.setpointDecimals = setpointDecimals
        c.setpointLabel = setpointLabel; c.setpointInPacket = setpointInPacket
        c.warning = warning
        return c
    }
}
