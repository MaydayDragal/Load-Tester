...
class El15BleManager(private val context: Context) : El15Controller {
    ...
    /**
     * Address of the last device we connected (or tried to connect) to, or null.
     * [LinkGuard] reconnects to this to force the load off after a link drop;
     * it falls back to the stored device when this session never connected.
     */
    ...
    private val gattCallback = object : BluetoothGattCallback() {
        ...
        override fun onDescriptorWrite(g: BluetoothGatt, d: BluetoothGattDescriptor, status: Int) {
            main.post {
                ...
                // Ask for the shortest connection interval the peer will grant
                // (7.5-15 ms rather than the default ~50 ms). The R-Test sweep
                // fits EVERY status packet, so the poll rate is what tightens the
                // resistance uncertainty — at the default interval a 50 ms poll
                // simply cannot be serviced. Advisory: the peripheral may refuse,
                // and nothing here depends on it succeeding.
                try {
