...
one — every bench result below came from it. `esp32-s3-lcd35b` is the
ESP32-S3-Touch-LCD-3.5**B** being transitioned to; it lives on branch
`claude/esp32-s3-touch-lcd-3.5` and its bring-up is **in progress** — panel,
touch-taps, PSRAM and the I²C bus are verified on real hardware; audio, SD and
touch scrolling are not (see [`S3_BRINGUP.md`](S3_BRINGUP.md)).

> The S3 hardware arrived 2026-08-10 and turned out to be the **3.5B**, not the
> plain 3.5 the port was originally written for. Different panel controller
> (AXS15231B over QSPI, not ST7796 over SPI) and four GPIOs assigned differently.
> The target was repointed; the plain 3.5 is no longer supported.
...
| Date | What happened |
|---|---|
| 2026-08-10 | **The S3 hardware arrived and is a 3.5B, not a 3.5 — the target was repointed and the panel now works.** The first flash rendered multi-coloured static: the 3.5B drives an **AXS15231B over QSPI**, not an ST7796 over 4-wire SPI, and four GPIOs are assigned differently (12 is the panel's QSPI CS, which the old header used for I²S MCLK — harmless only because audio was already failing). Nobody owns a plain 3.5, so `esp32-s3-lcd35` was repointed rather than a third target added; `board_s3_lcd35.h` is deleted and `board_config.h` `#error`s on the old macro. Three panel bugs, in the order they had to be peeled apart: touch was polling an empty 0x38 (it is at **0x3B**, and is a command/response part, not a register file) which buried the log in ~90 failed I²C transactions/second; the panel **will not accept a partial write**, fixed with LVGL `full_refresh` + full-frame PSRAM buffers, which is what Waveshare's own `Arduino_Canvas`-based demos effectively do; and `LCD_SPI_HZ` was over-clocked to 80 MHz by analogy with the C6 when the vendor runs the Arduino_GFX default of **40 MHz** — the symptom was every fourth pixel row displaced 2-3 px right. A boot-time I²C census and a PSRAM line were added so the next person reads the board's state instead of inferring it. **Verified on hardware:** boot, 8 MB PSRAM, all six I²C devices, the panel, and touch taps. **Still open:** touch scrolling (swipes fire the item under the finger), audio (an arduino-esp32 prebuilt-config defect — `CONFIG_GDMA_ISR_IRAM_SAFE=y` demands an internal-RAM user context while `CONFIG_I2S_ISR_IRAM_SAFE` is unset, so I²S allocates it in PSRAM), SD, and BLE connect. Arduino_GFX 1.6.7 already shipped both the AXS15231B driver and the QSPI bus, so the library pinning is untouched. |
